...
export function OrderCardCollapsible(props: {
  ...
  onSaveInternalNotes: (internalNotes: string) => Promise<boolean>
  ...
  const { order, expanded, onToggle, onChangeStatus, onSaveInternalNotes, showPosActions, onEditPos, onCancelPos, onReactivatePos } = props
  const [isInternalNotesOpen, setIsInternalNotesOpen] = useState(false)
  const [internalNotesDraft, setInternalNotesDraft] = useState(order.internalNotes ?? "")
  ...
  return (
    <>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      ...
                      <TableRow>
                        ...
                        <TableCell className="text-right font-medium text-muted-foreground">
                          {formatPrice(Number(order.shippingFee ?? 0))}
                        </TableCell>
                      </TableRow>
                      <TableRow>
                ...
                <div className="space-y-2">
                  {order.notes && (
                    <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
                      <p className="text-xs font-medium text-amber-800">Notas del cliente</p>
                      <p className="text-sm whitespace-pre-wrap text-amber-900">{order.notes}</p>
                    </div>
                  )}
                  {order.internalNotes && (
                    <div className="rounded-lg border border-sky-200 bg-sky-50 p-3">
                      <p className="text-xs font-medium text-sky-800">Notas internas</p>
                      <p className="text-sm whitespace-pre-wrap text-sky-900">{order.internalNotes}</p>
                    </div>
                  )}
                  <div>
                    <Button
      ...
      <Dialog open={isInternalNotesOpen} onOpenChange={setIsInternalNotesOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Notas internas</DialogTitle>
            <DialogDescription>Agrega o edita notas internas para este pedido.</DialogDescription>
          </DialogHeader>
          <Textarea
          ...
          <DialogFooter>
            <Button
              ...
              onClick={() => setIsInternalNotesOpen(false)}
            >
              Cancelar
            </Button>
            <Button
              ...
              onClick={async () => {
                ...
                const ok = await onSaveInternalNotes(internalNotesDraft)
                setIsSavingInternalNotes(false)
                if (ok) setIsInternalNotesOpen(false)
              }}
            >
              {isSavingInternalNotes ? "Guardando..." : "Guardar notas internas"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
