...
export async function GET() {
  const { data, error } = await supabaseAdmin
    .from("orders")
    .select(
      "id, order_number, customer_name, phone_number, status, origin, total, created_at, shipping_date, shipping_fee, notes, order_items (product_id, product_name, quantity, unit_price, line_total)"
    )
